...
static ParamBlockDesc2 LRI_Matte_param_blk (
	LRI_Matte_params, _T("params"),  0, GetLRI_MatteDesc(),	P_AUTO_CONSTRUCT + P_AUTO_UI, PBLOCK_REF, 
	//rollout
...
void LRI_Matte::SetSubMtl(int i, Mtl* m)
{
	...
	if (i == 0)
	{
		LRI_Matte_param_blk.InvalidateUI(mtl_mat1);
		mapValid.SetEmpty();
	}
}
...
void LRI_Matte::SetSubTexmap(int i, Texmap* tx)
{
	//mprintf(_T("\n SetSubTexmap Nubmer ============>>>  is : %i \n"), i);
	ReplaceReference(i +2, tx);
	if (i == 0)
	{
		LRI_Matte_param_blk.InvalidateUI(mtl_diffuse_map);
		mapValid.SetEmpty();
	}
	else
	{
		LRI_Matte_param_blk.InvalidateUI(mtl_bump_map);
		mapValid.SetEmpty();
	}
}
...
float LRI_Matte::EvalDisplacement(ShadeContext& sc)
{
	Mtl* subMaterial = mapOn[0] ? submtl[0] : nullptr;
	return (subMaterial) ? subMaterial->EvalDisplacement(sc) : 0.0f;
}

Interval LRI_Matte::DisplacementValidity(TimeValue t)
{
	Mtl* subMaterial = mapOn[0] ? submtl[0] : nullptr;
