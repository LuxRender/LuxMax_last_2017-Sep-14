...
#include "LR_Rough_Matte.h"
#include <maxscript\maxscript.h>

#define LR_Rough_Matte_CLASS_ID	Class_ID(0x34b56e70, 0x7de894e5)
...
class LR_Rough_Matte : public Mtl {
public:
	LR_Rough_Matte();
	LR_Rough_Matte(BOOL loading);
	~LR_Rough_Matte();
	...
	// From Animatable
	virtual Class_ID ClassID() {return LR_Rough_Matte_CLASS_ID;}
...
class LR_Rough_MatteClassDesc : public ClassDesc2 
{
public:
	virtual int IsPublic() 							{ return TRUE; }
	virtual void* Create(BOOL loading = FALSE) 		{ return new LR_Rough_Matte(loading); }
	virtual const TCHAR *	ClassName() 			{ return GetString(IDS_CLASS_NAME); }
	virtual SClass_ID SuperClassID() 				{ return MATERIAL_CLASS_ID; }
	virtual Class_ID ClassID() 						{ return LR_Rough_Matte_CLASS_ID; }
	virtual const TCHAR* Category() 				{ return GetString(IDS_CATEGORY); }

	virtual const TCHAR* InternalName() 			{ return _T("LR_Rough_Matte"); }	// returns fixed parsable name (scripter-visible name)
...
ClassDesc2* GetLR_Rough_MatteDesc() { 
	static LR_Rough_MatteClassDesc LR_Rough_MatteDesc;
	return &LR_Rough_MatteDesc; 
}
...
static ParamBlockDesc2 LR_Rough_Matte_param_blk (
	LR_Rough_Matte_params, _T("params"),  0, GetLR_Rough_MatteDesc(),	P_AUTO_CONSTRUCT + P_AUTO_UI, PBLOCK_REF, 
	//rollout
...
LR_Rough_Matte::LR_Rough_Matte(BOOL loading)
	: pblock(nullptr)
{
	for (int i = 0; i < NUM_SUBMATERIALS; i++)
	{
...
void LR_Rough_Matte::Reset()
{
	...
	GetLR_Rough_MatteDesc()->MakeAutoParamBlocks(this);
}



ParamDlg* LR_Rough_Matte::CreateParamDlg(HWND hwMtlEdit, IMtlParams *imp)
{
	IAutoMParamDlg* masterDlg = GetLR_Rough_MatteDesc()->CreateParamDlgs(hwMtlEdit, imp, this);
	// TODO: Set param block user dialog if necessary
	return masterDlg;
	
}

BOOL LR_Rough_Matte::SetDlgThing(ParamDlg* /*dlg*/)
{
	return FALSE;
}

Interval LR_Rough_Matte::Validity(TimeValue t)
{
...
RefTargetHandle LR_Rough_Matte::GetReference(int i)
{
	switch (i)
	{
...
void LR_Rough_Matte::SetReference(int i, RefTargetHandle rtarg)
{
...
TSTR LR_Rough_Matte::SubAnimName(int i)
{
...
Animatable* LR_Rough_Matte::SubAnim(int i)
{
	switch (i)
	{
...
RefResult LR_Rough_Matte::NotifyRefChanged(const Interval& /*changeInt*/, RefTargetHandle hTarget, 
	...
	case REFMSG_CHANGE:
		...
		mapValid.SetEmpty();
			if (hTarget == pblock)
			{
				...
				LR_Rough_Matte_param_blk.InvalidateUI(changing_param);
			}
		}
		break;
...
Mtl* LR_Rough_Matte::GetSubMtl(int i)
{
...
void LR_Rough_Matte::SetSubMtl(int i, Mtl* m)
{
	//mprintf(_T("\n SetSubMtl Nubmer is : %i \n"), i);
	ReplaceReference(i , m);
	if (i == 0)
	{
		LR_Rough_Matte_param_blk.InvalidateUI(mtl_mat1);
		mapValid.SetEmpty();
	}
}

TSTR LR_Rough_Matte::GetSubMtlSlotName(int i)
{
...
TSTR LR_Rough_Matte::GetSubMtlTVName(int i)
{
...
Texmap* LR_Rough_Matte::GetSubTexmap(int i)
{
...
void LR_Rough_Matte::SetSubTexmap(int i, Texmap* tx)
{
	//mprintf(_T("\n SetSubTexmap Nubmer ============>>>  is : %i \n"), i);
	ReplaceReference(i +2, tx);
	if (i == 0)
	{
		LR_Rough_Matte_param_blk.InvalidateUI(mtl_diffuse_map);
		mapValid.SetEmpty();
	}
	if (i == 1)
		{
			LR_Rough_Matte_param_blk.InvalidateUI(mtl_bump_map);
			mapValid.SetEmpty();
		}
	if (i == 2)
	{
		LR_Rough_Matte_param_blk.InvalidateUI(mtl_translucent_map);
		mapValid.SetEmpty();
	}
//	else
//	{
...
TSTR LR_Rough_Matte::GetSubTexmapSlotName(int i)
{
	switch (i)
	{
		case 0:
...
TSTR LR_Rough_Matte::GetSubTexmapTVName(int i)
{
...
IOResult LR_Rough_Matte::Save(ISave* isave)
{
	IOResult res;
...
IOResult LR_Rough_Matte::Load(ILoad* iload)
{
	IOResult res;
...
RefTargetHandle LR_Rough_Matte::Clone(RemapDir &remap)
{
	LR_Rough_Matte *mnew = new LR_Rough_Matte(FALSE);
...
void LR_Rough_Matte::NotifyChanged()
{
	NotifyDependents(FOREVER, PART_ALL, REFMSG_CHANGE);
}

void LR_Rough_Matte::Update(TimeValue t, Interval& valid)
{
...
void LR_Rough_Matte::SetAmbient(Color /*c*/, TimeValue /*t*/) {}		
void LR_Rough_Matte::SetDiffuse(Color /*c*/, TimeValue /*t*/) {}		
void LR_Rough_Matte::SetSpecular(Color /*c*/, TimeValue /*t*/) {}
void LR_Rough_Matte::SetShininess(float /*v*/, TimeValue /*t*/) {}

Color LR_Rough_Matte::GetAmbient(int mtlNum, BOOL backFace)
{
	Point3 p;
...
Color LR_Rough_Matte::GetDiffuse(int mtlNum, BOOL backFace)
{
	Point3 p;
	//TimeValue t; //Zero for first frame //GetCOREInterface()->GetTime() for every frame
...
Color LR_Rough_Matte::GetSpecular(int mtlNum, BOOL backFace)
{
	Point3 p;
	pblock->GetValue(prm_color, 0, p, ivalid);
...
float LR_Rough_Matte::GetXParency(int mtlNum, BOOL backFace)
{
	float t = 0.0f;
...
float LR_Rough_Matte::GetShininess(int mtlNum, BOOL backFace)
{
	float sh = 1.0f;
...
float LR_Rough_Matte::GetShinStr(int mtlNum, BOOL backFace)
{
	return submtl[0] ? submtl[0]->GetShinStr(mtlNum,backFace): 0.0f;
}

float LR_Rough_Matte::WireSize(int mtlNum, BOOL backFace)
{
	float wf = 0.0f;
...
void LR_Rough_Matte::Shade(ShadeContext& sc)
{
...
float LR_Rough_Matte::EvalDisplacement(ShadeContext& sc)
{
	Mtl* subMaterial = mapOn[0] ? submtl[0] : nullptr;
	return (subMaterial) ? subMaterial->EvalDisplacement(sc) : 0.0f;
}

Interval LR_Rough_Matte::DisplacementValidity(TimeValue t)
{
	Mtl* subMaterial = mapOn[0] ? submtl[0] : nullptr;
