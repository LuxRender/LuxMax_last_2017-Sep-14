...
#include "LR_Matte_Translucent.h"
#include <maxscript\maxscript.h>

#define LR_Matte_Translucent_CLASS_ID	Class_ID(0x31b26e70, 0x2de454e4)
...
class LR_Matte_Translucent : public Mtl {
public:
	LR_Matte_Translucent();
	LR_Matte_Translucent(BOOL loading);
	~LR_Matte_Translucent();
	...
	// From Animatable
	virtual Class_ID ClassID() {return LR_Matte_Translucent_CLASS_ID;}
...
class LR_Matte_TranslucentClassDesc : public ClassDesc2 
{
public:
	virtual int IsPublic() 							{ return TRUE; }
	virtual void* Create(BOOL loading = FALSE) 		{ return new LR_Matte_Translucent(loading); }
	virtual const TCHAR *	ClassName() 			{ return GetString(IDS_CLASS_NAME); }
	virtual SClass_ID SuperClassID() 				{ return MATERIAL_CLASS_ID; }
	virtual Class_ID ClassID() 						{ return LR_Matte_Translucent_CLASS_ID; }
	virtual const TCHAR* Category() 				{ return GetString(IDS_CATEGORY); }

	virtual const TCHAR* InternalName() 			{ return _T("LR_Matte_Translucent"); }	// returns fixed parsable name (scripter-visible name)
...
ClassDesc2* GetLR_Matte_TranslucentDesc() { 
	static LR_Matte_TranslucentClassDesc LR_Matte_TranslucentDesc;
	return &LR_Matte_TranslucentDesc; 
}
...
enum { LR_Matte_Translucent_params };
...
static ParamBlockDesc2 LR_Matte_Translucent_param_blk (
	LR_Matte_Translucent_params, _T("params"),  0, GetLR_Matte_TranslucentDesc(),	P_AUTO_CONSTRUCT + P_AUTO_UI, PBLOCK_REF, 
	//rollout
...
LR_Matte_Translucent::LR_Matte_Translucent()
	: pblock(nullptr)
{
...
LR_Matte_Translucent::LR_Matte_Translucent(BOOL loading)
	: pblock(nullptr)
{
	for (int i = 0; i < NUM_SUBMATERIALS; i++)
	{
...
void LR_Matte_Translucent::Reset()
{
	ivalid.SetEmpty();
	mapValid.SetEmpty();
	...
	GetLR_Matte_TranslucentDesc()->MakeAutoParamBlocks(this);
}



ParamDlg* LR_Matte_Translucent::CreateParamDlg(HWND hwMtlEdit, IMtlParams *imp)
{
	IAutoMParamDlg* masterDlg = GetLR_Matte_TranslucentDesc()->CreateParamDlgs(hwMtlEdit, imp, this);
	// TODO: Set param block user dialog if necessary
	return masterDlg;
	
}

BOOL LR_Matte_Translucent::SetDlgThing(ParamDlg* /*dlg*/)
{
	return FALSE;
}

Interval LR_Matte_Translucent::Validity(TimeValue t)
{
...
RefTargetHandle LR_Matte_Translucent::GetReference(int i)
{
	switch (i)
	{
...
TSTR LR_Matte_Translucent::SubAnimName(int i)
{
...
Animatable* LR_Matte_Translucent::SubAnim(int i)
{
	switch (i)
	{
...
RefResult LR_Matte_Translucent::NotifyRefChanged(const Interval& /*changeInt*/, RefTargetHandle hTarget, 
	...
	case REFMSG_CHANGE:
		...
		mapValid.SetEmpty();
			if (hTarget == pblock)
			{
				...
				LR_Matte_Translucent_param_blk.InvalidateUI(changing_param);
			}
		}
		break;
...
Mtl* LR_Matte_Translucent::GetSubMtl(int i)
{
...
void LR_Matte_Translucent::SetSubMtl(int i, Mtl* m)
{
	//mprintf(_T("\n SetSubMtl Nubmer is : %i \n"), i);
	ReplaceReference(i , m);
	if (i == 0)
	{
		LR_Matte_Translucent_param_blk.InvalidateUI(mtl_mat1);
		mapValid.SetEmpty();
	}
}

TSTR LR_Matte_Translucent::GetSubMtlSlotName(int i)
{
...
TSTR LR_Matte_Translucent::GetSubMtlTVName(int i)
{
...
TSTR LR_Matte_Translucent::GetSubTexmapTVName(int i)
{
...
IOResult LR_Matte_Translucent::Save(ISave* isave)
{
	IOResult res;
...
IOResult LR_Matte_Translucent::Load(ILoad* iload)
{
	IOResult res;
...
RefTargetHandle LR_Matte_Translucent::Clone(RemapDir &remap)
{
	LR_Matte_Translucent *mnew = new LR_Matte_Translucent(FALSE);
...
void LR_Matte_Translucent::NotifyChanged()
{
	NotifyDependents(FOREVER, PART_ALL, REFMSG_CHANGE);
}

void LR_Matte_Translucent::Update(TimeValue t, Interval& valid)
{
...
void LR_Matte_Translucent::SetAmbient(Color /*c*/, TimeValue /*t*/) {}		
void LR_Matte_Translucent::SetDiffuse(Color /*c*/, TimeValue /*t*/) {}		
void LR_Matte_Translucent::SetSpecular(Color /*c*/, TimeValue /*t*/) {}
void LR_Matte_Translucent::SetShininess(float /*v*/, TimeValue /*t*/) {}

Color LR_Matte_Translucent::GetAmbient(int mtlNum, BOOL backFace)
{
	Point3 p;
...
Color LR_Matte_Translucent::GetDiffuse(int mtlNum, BOOL backFace)
{
	Point3 p;
	//TimeValue t; //Zero for first frame //GetCOREInterface()->GetTime() for every frame
...
Color LR_Matte_Translucent::GetSpecular(int mtlNum, BOOL backFace)
{
	Point3 p;
	pblock->GetValue(prm_color, 0, p, ivalid);
...
float LR_Matte_Translucent::GetXParency(int mtlNum, BOOL backFace)
{
	float t = 0.0f;
...
float LR_Matte_Translucent::GetShininess(int mtlNum, BOOL backFace)
{
	float sh = 1.0f;
...
float LR_Matte_Translucent::GetShinStr(int mtlNum, BOOL backFace)
{
	return submtl[0] ? submtl[0]->GetShinStr(mtlNum,backFace): 0.0f;
}

float LR_Matte_Translucent::WireSize(int mtlNum, BOOL backFace)
{
	float wf = 0.0f;
...
void LR_Matte_Translucent::Shade(ShadeContext& sc)
{
...
float LR_Matte_Translucent::EvalDisplacement(ShadeContext& sc)
{
	Mtl* subMaterial = mapOn[0] ? submtl[0] : nullptr;
	return (subMaterial) ? subMaterial->EvalDisplacement(sc) : 0.0f;
}

Interval LR_Matte_Translucent::DisplacementValidity(TimeValue t)
{
	Mtl* subMaterial = mapOn[0] ? submtl[0] : nullptr;
