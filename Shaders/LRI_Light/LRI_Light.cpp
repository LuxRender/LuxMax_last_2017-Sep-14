...
class LRI_Light : public Mtl {
public:
	LRI_Light();
	LRI_Light(BOOL loading);
	~LRI_Light();
	...
	// From Animatable
	virtual Class_ID ClassID() {return LRI_Light_CLASS_ID;}
...
class LRI_LightClassDesc : public ClassDesc2 
{
public:
	virtual int IsPublic() 							{ return TRUE; }
	virtual void* Create(BOOL loading = FALSE) 		{ return new LRI_Light(loading); }
	virtual const TCHAR *	ClassName() 			{ return GetString(IDS_CLASS_NAME); }
	virtual SClass_ID SuperClassID() 				{ return MATERIAL_CLASS_ID; }
	virtual Class_ID ClassID() 						{ return LRI_Light_CLASS_ID; }
	virtual const TCHAR* Category() 				{ return GetString(IDS_CATEGORY); }

	virtual const TCHAR* InternalName() 			{ return _T("LRI_Light"); }	// returns fixed parsable name (scripter-visible name)
...
ClassDesc2* GetLRI_LightDesc() { 
	static LRI_LightClassDesc LRI_LightDesc;
	return &LRI_LightDesc; 
}
...
static ParamBlockDesc2 LRI_Light_param_blk ( LRI_Light_params, _T("params"),  0, GetLRI_LightDesc(), 
...
LRI_Light::LRI_Light(BOOL loading)
	: pblock(nullptr)
{
	for (int i=0; i<NUM_SUBMATERIALS; i++) 
		submtl[i] = nullptr;
...
void LRI_Light::Reset()
{
	...
	GetLRI_LightDesc()->MakeAutoParamBlocks(this);
}
...
BOOL LRI_Light::SetDlgThing(ParamDlg* /*dlg*/)
{
	return FALSE;
}

Interval LRI_Light::Validity(TimeValue t)
{
...
RefTargetHandle LRI_Light::GetReference(int i)
{
...
void LRI_Light::SetReference(int i, RefTargetHandle rtarg)
{
	if ((i >= 0) && (i < NUM_SUBMATERIALS))
		submtl[i] = (Mtl *)rtarg;
	else if (i == PBLOCK_REF)
...
TSTR LRI_Light::SubAnimName(int i)
{
	if ((i >= 0) && (i < NUM_SUBMATERIALS))
...
Animatable* LRI_Light::SubAnim(int i)
{
	if ((i >= 0) && (i < NUM_SUBMATERIALS))
		return submtl[i];
	else if (i == PBLOCK_REF)
...
RefResult LRI_Light::NotifyRefChanged(const Interval& /*changeInt*/, RefTargetHandle hTarget, 
	...
	case REFMSG_CHANGE:
		{
			...
			if (hTarget == pblock)
			{
				...
				LRI_Light_param_blk.InvalidateUI(changing_param);
			}
		}
		break;
...
void LRI_Light::SetSubMtl(int i, Mtl* m)
{
	ReplaceReference(i,m);
	// TODO: Set the material and update the UI
}

TSTR LRI_Light::GetSubMtlSlotName(int)
{
	// Return i'th sub-material name
	return _T("");
}

TSTR LRI_Light::GetSubMtlTVName(int i)
{
...
Texmap* LRI_Light::GetSubTexmap(int /*i*/)
{
	return nullptr;
}

void LRI_Light::SetSubTexmap(int /*i*/, Texmap* /*m*/)
{
}

TSTR LRI_Light::GetSubTexmapSlotName(int /*i*/)
{
	return _T("");
}

TSTR LRI_Light::GetSubTexmapTVName(int i)
{
...
IOResult LRI_Light::Save(ISave* isave)
{
	IOResult res;
...
IOResult LRI_Light::Load(ILoad* iload)
{
	IOResult res;
...
RefTargetHandle LRI_Light::Clone(RemapDir &remap)
{
	LRI_Light *mnew = new LRI_Light(FALSE);
...
void LRI_Light::NotifyChanged()
{
	NotifyDependents(FOREVER, PART_ALL, REFMSG_CHANGE);
}

void LRI_Light::Update(TimeValue t, Interval& valid)
{
...
void LRI_Light::SetAmbient(Color /*c*/, TimeValue /*t*/) {}		
void LRI_Light::SetDiffuse(Color /*c*/, TimeValue /*t*/) {}		
void LRI_Light::SetSpecular(Color /*c*/, TimeValue /*t*/) {}
void LRI_Light::SetShininess(float /*v*/, TimeValue /*t*/) {}

Color LRI_Light::GetAmbient(int mtlNum, BOOL backFace)
{
	Point3 p;
...
Color LRI_Light::GetDiffuse(int mtlNum, BOOL backFace)
{
	Point3 p;
	//TimeValue t; //Zero for first frame //GetCOREInterface()->GetTime() for every frame
...
Color LRI_Light::GetSpecular(int mtlNum, BOOL backFace)
{
	return submtl[0] ? submtl[0]->GetSpecular(mtlNum,backFace): Color(1,1,1);
}

float LRI_Light::GetXParency(int mtlNum, BOOL backFace)
{
	return submtl[0] ? submtl[0]->GetXParency(mtlNum,backFace): 0.0f;
}

float LRI_Light::GetShininess(int mtlNum, BOOL backFace)
{
	return submtl[0] ? submtl[0]->GetShininess(mtlNum,backFace): 1.0f;
}

float LRI_Light::GetShinStr(int mtlNum, BOOL backFace)
{
	return submtl[0] ? submtl[0]->GetShinStr(mtlNum,backFace): 0.0f;
}

float LRI_Light::WireSize(int mtlNum, BOOL backFace)
{
...
void LRI_Light::Shade(ShadeContext& sc)
{
